...
final readonly class ObjectBackendMigrationCommandReport
{
    ...
    public function __construct(
        private ObjectBackendMigrationCommandPacket $packet,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting backend migration command check cannot be empty.');
            }
        }
        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting backend migration command blocking reason cannot be empty.');
            }
        }
    }

    public function packet(): ObjectBackendMigrationCommandPacket
    {
        return $this->packet;
    }
