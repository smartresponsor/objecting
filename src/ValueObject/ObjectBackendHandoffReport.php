...
final readonly class ObjectBackendHandoffReport
{
    ...
    public function __construct(
        private ObjectBackendHandoffManifest $manifest,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting backend handoff check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting backend handoff blocking reason cannot be empty.');
            }
        }
    }

    public function manifest(): ObjectBackendHandoffManifest
    {
