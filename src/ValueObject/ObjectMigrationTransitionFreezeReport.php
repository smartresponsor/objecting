...
final readonly class ObjectMigrationTransitionFreezeReport
{
    ...
    public function __construct(
        private ObjectMigrationTransitionFreezeManifest $manifest,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting migration transition freeze check cannot be empty.');
            }
        }
        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting migration transition freeze blocking reason cannot be empty.');
            }
        }
    }

    public function manifest(): ObjectMigrationTransitionFreezeManifest
    {
