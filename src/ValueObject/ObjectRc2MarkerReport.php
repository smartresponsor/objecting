...
final readonly class ObjectRc2MarkerReport
{
    ...
    public function __construct(
        private ObjectRc2MarkerManifest $manifest,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting RC2 marker check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting RC2 marker blocking reason cannot be empty.');
            }
        }
    }

    public function manifest(): ObjectRc2MarkerManifest
    {
