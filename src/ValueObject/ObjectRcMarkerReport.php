...
final readonly class ObjectRcMarkerReport
{
    ...
    public function __construct(
        private ObjectRcMarkerManifest $manifest,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting RC marker check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting RC marker blocking reason cannot be empty.');
            }
        }
    }

    public function manifest(): ObjectRcMarkerManifest
    {
