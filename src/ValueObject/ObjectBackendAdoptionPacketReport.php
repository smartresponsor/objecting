...
final readonly class ObjectBackendAdoptionPacketReport
{
    ...
    public function __construct(
        private ObjectBackendAdoptionPacketManifest $manifest,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting backend adoption packet check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting backend adoption packet blocking reason cannot be empty.');
            }
        }
    }

    public function manifest(): ObjectBackendAdoptionPacketManifest
    {
