...
final readonly class ObjectReleaseClosureReport
{
    ...
    public function __construct(
        private ObjectReleaseClosureManifest $manifest,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting release closure check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting release closure blocking reason cannot be empty.');
            }
        }
    }

    public function manifest(): ObjectReleaseClosureManifest
    {
