...
final readonly class ObjectBackendImportReport
{
    ...
    public function __construct(
        private ObjectBackendImportContract $contract,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting backend import check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting backend import blocking reason cannot be empty.');
            }
        }
    }

    public function contract(): ObjectBackendImportContract
    {
