...
final readonly class ObjectRcStabilizationReport
{
    ...
    public function __construct(
        private ObjectRcStabilizationManifest $manifest,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting RC stabilization check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting RC stabilization blocking reason cannot be empty.');
            }
        }
    }

    public function manifest(): ObjectRcStabilizationManifest
    {
