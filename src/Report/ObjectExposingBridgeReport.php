...
final readonly class ObjectExposingBridgeReport
{
    ...
    public function __construct(
        private ObjectExposingBridgeContract $contract,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting Exposing bridge check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting Exposing bridge blocking reason cannot be empty.');
            }
        }
    }

    public function contract(): ObjectExposingBridgeContract
    {
