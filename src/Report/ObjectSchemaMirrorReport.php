...
final readonly class ObjectSchemaMirrorReport
{
    ...
    public function __construct(
        private ObjectSchemaMirrorContract $contract,
        private array $checks,
        private array $blockingReasons,
    ) {
        foreach ($this->checks as $check) {
            if ('' === $check) {
                throw new \InvalidArgumentException('Objecting schema mirror check cannot be empty.');
            }
        }

        foreach ($this->blockingReasons as $blockingReason) {
            if ('' === $blockingReason) {
                throw new \InvalidArgumentException('Objecting schema mirror blocking reason cannot be empty.');
            }
        }
    }

    public function contract(): ObjectSchemaMirrorContract
    {
